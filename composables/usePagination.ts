...
export function usePagination(source: ComputedRef<string>) {
  ...
  function doMeasure(body: any, retries = 0) {
    ...
    // Sanity check: if all children have zero height, MDC hasn't rendered yet
    const totalHeight = children.reduce((sum, c) => sum + c.getBoundingClientRect().height, 0)
    if (totalHeight < 10 && retries < MAX_RETRIES) {
      const delay = 50 + retries * 60
      setTimeout(() => doMeasure(body, retries + 1), delay)
      return
    }
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (accum + h > CONTENT && currentEntries.length > 0) {
        // Element doesn't fit on current page — move entire element to next page
